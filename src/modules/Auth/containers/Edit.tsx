...
export function Edit(props: RouterProps) {
    ...
    const { history } = props
    const [editState, setEditState] = React.useState({ email : '', password : '', error : '', salutation : '',  first_name : '',  middle_name : '',  last_name : '',  address_line_1 : '',  address_line_2 : '',  city : '',  state : '',  country : '',  pincode : '',  phone_number : '',  secondary_phone : '', recoveryAnswer : '', passwordconfirm : ''});
    function onChange(e: any) {
        console.log(e.target.name, e.target.value);
        setEditState({ ...editState, [e.target.name]: e.target.value.trim() })
    }
    async function onSubmit() {
        if(editState.first_name.length && editState.last_name.length && editState.address_line_1.length && editState.city.length && editState.state.length && editState.country.length && editState.pincode.length && editState.phone_number.length && editState.recoveryAnswer.length && editState.passwordconfirm.length){
        ...
        <Flex direction="column" align="center" height="100%" pt="5vh">
            ...
            <Flex direction="column" border="1px solid #d5d5d5" p="max(2vmax, 15px)" borderRadius="15">
                <>        
                    ...
                    <Input placeholder="First Name" name="first_name" onChange={onChange} value={editState.first_name} mb={1} />
                    <Input placeholder="Middle Name" name="middle_name" onChange={onChange} value={editState.middle_name} mb={1} />
                    <Input placeholder="Last Name" name="last_name" onChange={onChange} value={editState.last_name} mb={1} />
                    <Input placeholder="Address Line 1" name="address_line_1" onChange={onChange} value={editState.address_line_1} mb={1} />
                    <Input placeholder="Address Line 2 (Optional)" name="address_line_2" onChange={onChange} value={editState.address_line_2} mb={1} />
                    ...
                    <NumberInput>
                        <NumberInputField placeholder="Mobile Primary Number" name="phone_number" onChange={onChange} value={editState.phone_number} mb={1} />
                    </NumberInput>
                    <NumberInput>
                        <NumberInputField  placeholder="Mobile Secondary Number" name="secondary_phone" onChange={onChange} value={editState.secondary_phone} mb={1}/>
                    </NumberInput> 
                </>
